...
try {
    ...
    if (Test-Path "dist\SmartHAIOApp.exe") {
        Write-Host "✓ Build completed successfully!" -ForegroundColor Green
        Write-Host ""
        Write-Host "Executable location: dist\SmartHAIOApp.exe" -ForegroundColor Cyan
        
        # Get file size
        $fileSize = (Get-Item "dist\SmartHAIOApp.exe").Length / 1MB
...
Write-Host ""
Write-Host "🎉 Build completed successfully!" -ForegroundColor Green
Write-Host "You can now run: dist\SmartHAIOApp.exe" -ForegroundColor Cyan
Write-Host ""
